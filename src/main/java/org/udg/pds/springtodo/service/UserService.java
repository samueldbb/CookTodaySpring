...
public class UserService {
    ...
    public void removeReceptaPreferits(Long userId, Long receptaId)
    {
        User u = getUser(userId);
        Recepta recepta = receptaService.getRecepta(receptaId);
        Boolean aux = u.getReceptesPreferides().contains(recepta);
        if (aux) u.removeReceptaPreferida(recepta);
        userRepository.save(u);
    }

    public Collection<Recepta> getReceptesPreferides(Long id){
        User u = getUser(id);
        return u.getReceptesPreferides();
    }
}
